...
check_uuid_exists() {
    ...
    local existing_uuids=$(grep -o '"id":[[:space:]]*"[^"]*"' /etc/xray/config.json | cut -d'"' -f4 2>/dev/null)
...
if [[ "$1" == "--interactive" ]] || [[ "$1" == "-i" ]]; then
    ...
    create_trojan_account "$user" "$uuid" "$quota" "$iplimit" "$duration"
else
    ...
    # Eksekusi pembuatan account Trojan
    create_trojan_account "$user" "$uuid" "$quota" "$iplimit" "$duration"
fi