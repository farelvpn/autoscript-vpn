...
for username in "${expired_accounts[@]}"; do
    ...
    if [[ -f "/etc/xray/database/trojan/${username}.txt" ]]; then
        mv "/etc/xray/database/trojan/${username}.txt" "/etc/xray/recovery/trojan/${username}.txt"
        echo "  ✓ Database file moved to recovery"
    fi
    
    # Remove quota limit file if exists
    if [[ -f "/etc/xray/limit/quota/trojan/${username}" ]]; then
        rm -f "/etc/xray/limit/quota/trojan/${username}"
        echo "  ✓ Quota limit file removed"
    fi
    
    # Remove IP limit file if exists
    if [[ -f "/etc/xray/limit/ip/trojan/${username}" ]]; then
        rm -f "/etc/xray/limit/ip/trojan/${username}"
        echo "  ✓ IP limit file removed"
    fi
    
    # Remove usage quota file if exists
    if [[ -f "/etc/xray/usage/quota/trojan/${username}" ]]; then
        rm -f "/etc/xray/usage/quota/trojan/${username}"
...
echo "Recovery files are available in: /etc/xray/recovery/trojan/"
