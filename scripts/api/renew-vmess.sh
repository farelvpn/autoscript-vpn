...
# ========================================================
# ========================================================
# RENEW VMESS ACCOUNT EXPIRATION (JSON API)
# ========================================================
...
# Function to renew VMESS account
renew_vmess_account() {
    local renew_user="$1"
    local renew_days="$2"
    ...
    user_uuid=$(grep "^uuid:" "/etc/xray/database/vmess/${renew_user}.txt" | cut -d' ' -f2-)
    ...
    if ! sed -i "s/expired:.*/expired: $new_exp/" "/etc/xray/database/vmess/${renew_user}.txt" 2>/dev/null; then
    ...
    response=$(cat <<EOF
{
    "status": "true",
    "code": 200,
    "message": "VMESS account '$renew_user' renewed successfully",
    "data": {
        "username": "$renew_user",
        "uuid": "$user_uuid",
...
if [[ "$1" == "--interactive" ]] || [[ "$1" == "-i" ]]; then
    ...
    renew_vmess_account "$renew_user" "$renew_days"
else
    ...
    # Eksekusi perpanjangan masa aktif account VMESS
    renew_vmess_account "$renew_user" "$renew_days"
fi