...
echo -e "\e[0;41;36m                    MEMBER XRAY TROJAN                       \e[0m"
echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\e[037;1m"
...
for user_file in $user_files; do
    if [[ -f "/etc/xray/database/trojan/$user_file" ]]; then
        ...
        uuid=$(grep "^uuid:" "/etc/xray/database/trojan/$user_file" | cut -d' ' -f2-)
        limit_ip_raw=$(grep "^limit_ip:" "/etc/xray/database/trojan/$user_file" | cut -d' ' -f2-)
        quota_raw=$(grep "^quota:" "/etc/xray/database/trojan/$user_file" | cut -d' ' -f2-)
        expired=$(grep "^expired:" "/etc/xray/database/trojan/$user_file" | cut -d' ' -f2-)
        
        # Format values
        days_left=$(calculate_days_remaining "$expired")
        limit_ip=$(format_ip_limit "/etc/xray/limit/ip/trojan/$username")
        quota=$(format_quota "/etc/xray/limit/quota/trojan/$username")
