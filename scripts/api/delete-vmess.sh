...
# ========================================================
# ========================================================
# DELETE VMESS ACCOUNT WITH RECOVERY (JSON API)
# ========================================================
...
delete_vmess_account() {
    ...
    if [[ -f "/etc/xray/database/vmess/${delete_user}.txt" ]]; then
        user_data=$(cat "/etc/xray/database/vmess/${delete_user}.txt")
        user_uuid=$(grep "^uuid:" "/etc/xray/database/vmess/${delete_user}.txt" | cut -d' ' -f2-)
        user_expired=$(grep "^expired:" "/etc/xray/database/vmess/${delete_user}.txt" | cut -d' ' -f2-)
    fi
    ...
    if [[ -f "/etc/xray/database/vmess/${delete_user}.txt" ]]; then
        if mv "/etc/xray/database/vmess/${delete_user}.txt" "/etc/xray/recovery/vmess/${delete_user}.txt" 2>/dev/null; then
    ...
    if [[ -f "/etc/xray/limit/quota/vmess/${delete_user}" ]]; then
        if rm -f "/etc/xray/limit/quota/vmess/${delete_user}" 2>/dev/null; then
    ...
    if [[ -f "/etc/xray/limit/ip/vmess/${delete_user}" ]]; then
        if rm -f "/etc/xray/limit/ip/vmess/${delete_user}" 2>/dev/null; then
    ...
    if [[ -f "/etc/xray/usage/quota/vmess/${delete_user}" ]]; then
        if rm -f "/etc/xray/usage/quota/vmess/${delete_user}" 2>/dev/null; then
    ...
    response=$(cat <<EOF
{
    "status": "true",
    "code": 200,
    "message": "VMESS account '$delete_user' deleted successfully",
    "data": {
        ...
        "recovery": {
            ...
            "path": "/etc/xray/recovery/vmess/${delete_user}.txt"
        },
...
if [[ "$1" == "--interactive" ]] || [[ "$1" == "-i" ]]; then
    ...
    delete_vmess_account "$delete_user"
else
    ...
    # Eksekusi penghapusan account VMESS
    delete_vmess_account "$delete_user" "$force_delete"
fi