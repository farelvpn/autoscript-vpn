...
# ========================================================
# ========================================================
# DELETE VLESS ACCOUNT WITH RECOVERY (JSON API)
# ========================================================
...
delete_vless_account() {
    ...
    if [[ ! -f "/etc/xray/database/vless/${delete_user}.txt" ]]; then
    ...
    if [[ -f "/etc/xray/database/vless/${delete_user}.txt" ]]; then
        user_data=$(cat "/etc/xray/database/vless/${delete_user}.txt")
        user_uuid=$(grep "^uuid:" "/etc/xray/database/vless/${delete_user}.txt" | cut -d' ' -f2-)
        user_expired=$(grep "^expired:" "/etc/xray/database/vless/${delete_user}.txt" | cut -d' ' -f2-)
    fi
    ...
    if [[ -f "/etc/xray/database/vless/${delete_user}.txt" ]]; then
        if mv "/etc/xray/database/vless/${delete_user}.txt" "/etc/xray/recovery/vless/${delete_user}.txt" 2>/dev/null; then
    ...
    if [[ -f "/etc/xray/limit/quota/vless/${delete_user}" ]]; then
        if rm -f "/etc/xray/limit/quota/vless/${delete_user}" 2>/dev/null; then
            files_removed+=("quota_limit")
        fi
    fi

    # Remove IP limit file if exists
    if [[ -f "/etc/xray/limit/ip/vless/${delete_user}" ]]; then
        if rm -f "/etc/xray/limit/ip/vless/${delete_user}" 2>/dev/null; then
            files_removed+=("ip_limit")
        fi
    fi

    # Remove usage quota file if exists
    if [[ -f "/etc/xray/usage/quota/vless/${delete_user}" ]]; then
        if rm -f "/etc/xray/usage/quota/vless/${delete_user}" 2>/dev/null; then
    ...
    response=$(cat <<EOF
{
    "status": "true",
    "code": 200,
    "message": "VLESS account '$delete_user' deleted successfully",
    "data": {
        ...
        "recovery": {
            ...
            "path": "/etc/xray/recovery/vless/${delete_user}.txt"
        },
...
if [[ "$1" == "--interactive" ]] || [[ "$1" == "-i" ]]; then
    ...
    delete_vless_account "$delete_user"
else
    ...
    # Eksekusi penghapusan account VLESS
    delete_vless_account "$delete_user" "$force_delete"
fi