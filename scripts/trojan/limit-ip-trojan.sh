...
# ========================================================
# ========================================================
# IP LIMIT CHECKER FOR TROJAN ACCOUNTS
# ========================================================
...
VIOL_DIR="/etc/xray/limit/ip/trojan/.violations"
mkdir -p "$VIOL_DIR"
...
while true; do
    ...
    for akun in "${data[@]}"; do
        ...
        if [[ -f "/etc/xray/limit/ip/trojan/$akun" ]]; then
            limit_ip=$(cat "/etc/xray/limit/ip/trojan/$akun")
            
            # Only check if limit is greater than 0
            if [[ "$limit_ip" -gt 0 ]]; then
                # Get current active IPs for this user
                temp_log="/tmp/trojan_${akun}_iplimit.tmp"
                temp_ips="/tmp/trojan_${akun}_current_ips.tmp"
                temp_networks="/tmp/trojan_${akun}_networks.tmp"
                ...
                if [[ -s "$temp_log" ]]; then
                    ...
                    if [[ "$current_networks" -gt "$limit_ip" ]]; then
                        ...
                        if [[ -f "/etc/xray/database/trojan/${akun}.txt" ]]; then
                            mv "/etc/xray/database/trojan/${akun}.txt" "/etc/xray/recovery/trojan/${akun}.txt"
                        fi
                        
                        # Remove quota limit file if exists
                        if [[ -f "/etc/xray/limit/quota/trojan/${akun}" ]]; then
                            rm -f "/etc/xray/limit/quota/trojan/${akun}"
                        fi
                        
                        # Remove usage quota file if exists
                        if [[ -f "/etc/xray/usage/quota/trojan/${akun}" ]]; then
                            rm -f "/etc/xray/usage/quota/trojan/${akun}"
                        fi
                        
                        # Remove IP limit file
                        if [[ -f "/etc/xray/limit/ip/trojan/${akun}" ]]; then
                            rm -f "/etc/xray/limit/ip/trojan/${akun}"
                        fi
