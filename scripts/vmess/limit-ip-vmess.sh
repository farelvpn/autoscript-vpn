...
# ========================================================
# ========================================================
# IP LIMIT CHECKER FOR VMESS ACCOUNTS
# ========================================================
...
VIOL_DIR="/etc/xray/limit/ip/vmess/.violations"
mkdir -p "$VIOL_DIR"
...
while true; do
    ...
    for akun in "${data[@]}"; do
        ...
        if [[ -f "/etc/xray/limit/ip/vmess/$akun" ]]; then
            limit_ip=$(cat "/etc/xray/limit/ip/vmess/$akun")
            
            # Only check if limit is greater than 0
            if [[ "$limit_ip" -gt 0 ]]; then
                # Get current active IPs for this user
                temp_log="/tmp/vmess_${akun}_iplimit.tmp"
                temp_ips="/tmp/vmess_${akun}_current_ips.tmp"
                temp_networks="/tmp/vmess_${akun}_networks.tmp"
                ...
                if [[ -s "$temp_log" ]]; then
                    ...
                    if [[ "$current_networks" -gt "$limit_ip" ]]; then
                        ...
                        if [[ -f "/etc/xray/database/vmess/${akun}.txt" ]]; then
                            mv "/etc/xray/database/vmess/${akun}.txt" "/etc/xray/recovery/vmess/${akun}.txt"
                        fi
                        
                        # Remove quota limit file if exists
                        if [[ -f "/etc/xray/limit/quota/vmess/${akun}" ]]; then
                            rm -f "/etc/xray/limit/quota/vmess/${akun}"
                        fi
                        
                        # Remove usage quota file if exists
                        if [[ -f "/etc/xray/usage/quota/vmess/${akun}" ]]; then
                            rm -f "/etc/xray/usage/quota/vmess/${akun}"
                        fi
                        
                        # Remove IP limit file
                        if [[ -f "/etc/xray/limit/ip/vmess/${akun}" ]]; then
                            rm -f "/etc/xray/limit/ip/vmess/${akun}"
                        fi
