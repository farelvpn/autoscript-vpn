...
# Description: Renew TROJAN account
...
if ! db_account_exists "trojan" "$user"; then err "User not found."; read -n1 -s -r -p "Press any key..."; menu; exit 1; fi

new_epoch=$(acc_xray_renew "trojan" "$user" "$days")
if [[ -n "$new_epoch" ]]; then
  new_disp=$(date -d "@${new_epoch}" +"%d-%m-%Y %H:%M:%S")
  tg_send "<b>[ TROJAN RENEWED ]</b>%0AUsername: <code>${user}</code>%0ANew expiry: <code>${new_disp}</code>"
