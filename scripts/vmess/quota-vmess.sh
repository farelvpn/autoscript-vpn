...
while true; do
  ...
  if [[ ! -e /etc/xray/usage/quota/vmess ]]; then
    mkdir -p /etc/xray/usage/quota/vmess
  fi
  
  if [[ ! -e /etc/xray/limit/quota/vmess ]]; then
  ...
  do
    ...
    if [[ ${getThis} != "failed" ]]; then
      ...
      # Store current usage in /etc/xray/usage/quota/vmess/$username
      if [ -e /etc/xray/usage/quota/vmess/${user} ]; then
        current_usage=$(cat /etc/xray/usage/quota/vmess/${user});
        if [[ ${#current_usage} -gt 0 ]]; then
          total_usage=$(( ${downlink} + ${current_usage} ));
          echo "${total_usage}" > /etc/xray/usage/quota/vmess/"${user}"
        else
          echo "${downlink}" > /etc/xray/usage/quota/vmess/"${user}"
        fi
      else
        echo "${downlink}" > /etc/xray/usage/quota/vmess/"${user}"
      fi
  ...
  do
    ...
    if [ -e /etc/xray/limit/quota/vmess/${user} ]; then
      limit_quota=$(cat /etc/xray/limit/quota/vmess/${user});
      ...
      if [ -e /etc/xray/usage/quota/vmess/${user} ]; then
        ...
        if [[ ${current_usage} -gt ${limit_quota} ]]; then
          ...
          if [[ -f "/etc/xray/database/vmess/${user}.txt" ]]; then
            mv "/etc/xray/database/vmess/${user}.txt" "/etc/xray/recovery/vmess/${user}.txt"
          fi
          
          # Remove usage quota file
          if [[ -f "/etc/xray/usage/quota/vmess/${user}" ]]; then
            rm -f "/etc/xray/usage/quota/vmess/${user}"
          fi
          
          # Remove IP limit file if exists
          if [[ -f "/etc/xray/limit/ip/vmess/${user}" ]]; then
            rm -f "/etc/xray/limit/ip/vmess/${user}"
          fi
          ...
          if [[ -n "$BOT_TOKEN" && -n "$CHAT_ID" ]]; then
              ...
              TEXT+="<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>%0A"
              TEXT+="<b>Protocol     :</b> <code>VMESS</code>%0A"
