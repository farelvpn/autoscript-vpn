...
# ========================================================
# ========================================================
# TRIAL VLESS ACCOUNT CREATOR (JSON API)
# ========================================================
...
generate_random_username() {
    ...
    while [[ $(grep -w "\"$username\"" /etc/xray/config.json 2>/dev/null | wc -l) -gt 0 ]] || 
          [[ -f "/etc/xray/database/vless/${username}.txt" ]]; do
...
create_trial_vless_account() {
    ...
    # Add user to config
    if ! sed -i '/#vless$/a\#÷ '"$user $exp_full"'\
},{"id": "'"$uuid"'","email": "'"$user"'"' /etc/xray/config.json 2>/dev/null; then
    ...
    response=$(cat <<EOF
{
    ...
    "data": {
        ...
        "links": {
            "vless_ws_tls": "$vlesslink1",
            "vless_ws_http": "$vlesslink2"
        },
        "account_type": "trial",
...
if [[ "$1" == "--interactive" ]] || [[ "$1" == "-i" ]]; then
    ...
    create_trial_vless_account "$duration" "$custom_prefix"
else
    ...
    # Eksekusi pembuatan trial account VLESS
    create_trial_vless_account "$duration" "$custom_prefix"
fi