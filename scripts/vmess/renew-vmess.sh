...
# Description: Renew VMESS account
...
if ! db_account_exists "vmess" "$user"; then err "User not found."; read -n1 -s -r -p "Press any key..."; menu; exit 1; fi

new_epoch=$(acc_xray_renew "vmess" "$user" "$days")
if [[ -n "$new_epoch" ]]; then
  new_disp=$(date -d "@${new_epoch}" +"%d-%m-%Y %H:%M:%S")
  tg_send "<b>[ VMESS RENEWED ]</b>%0AUsername: <code>${user}</code>%0ANew expiry: <code>${new_disp}</code>"
