...
# Description: Recover (restore) deleted/suspended TROJAN account
...
clear
line
echo -e "${WHITE}  RECOVERY TROJAN ACCOUNT${NC}"
line
...
while IFS='|' read -r u e s; do
  ...
  total=$((total+1))
                   ...
                   FROM accounts WHERE protocol='trojan' AND status IN ('deleted','suspended')
                   ORDER BY updated_at DESC;")
line
if [[ $total -eq 0 ]]; then
  warn "No recoverable TROJAN accounts."
...
cnt=$(db_query "SELECT COUNT(*) FROM accounts WHERE protocol='trojan' AND username='$(sql_escape "$user")' AND status IN ('deleted','suspended');")
...
uuid=$(db_query "SELECT secret FROM accounts WHERE protocol='trojan' AND username='$(sql_escape "$user")' AND status IN ('deleted','suspended') ORDER BY updated_at DESC LIMIT 1;")
if cfg_add_client "trojan" "$user" "$uuid"; then
  db_exec "UPDATE accounts SET status='active', updated_at=strftime('%s','now')
           WHERE protocol='trojan' AND username='$(sql_escape "$user")';"
  db_audit "recover" "trojan" "$user" ""
