...
# Description: VMESS quota enforcement loop (DB-driven)
...
while true; do
  ...
  while IFS='|' read -r user quota used; do
    ...
    if [[ "$used" -ge "$quota" ]]; then
      acc_xray_suspend "vmess" "$user" "quota"
      tg_send "<b>[ VMESS QUOTA EXCEEDED ]</b>%0AUsername: <code>${user}</code>%0AStatus: suspended"
    fi
  done < <(db_query "SELECT username, quota_bytes, used_bytes FROM accounts
                     WHERE protocol='vmess' AND status='active';")
done
