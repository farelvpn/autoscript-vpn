...
# ========================================================
# ========================================================
# RENEW TROJAN ACCOUNT EXPIRATION (JSON API)
# ========================================================
...
show_trojan_accounts() {
    ...
    echo -e "\e[0;41;36m              RENEW TROJAN ACCOUNT EXPIRATION               \e[0m"
    echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\e[037;1m"
...
# Function to renew Trojan account
renew_trojan_account() {
    ...
    if [[ ! -f "/etc/xray/database/trojan/${renew_user}.txt" ]]; then
        echo "{\"status\": \"false\", \"code\": 404, \"message\": \"User '$renew_user' not found in database\"}"
        return 1
    fi

    # Get current expiration date from database
    current_exp=$(grep "^expired:" "/etc/xray/database/trojan/${renew_user}.txt" | cut -d' ' -f2-)
    ...
    user_uuid=$(grep "^uuid:" "/etc/xray/database/trojan/${renew_user}.txt" | cut -d' ' -f2-)
    ...
    if ! sed -i "s/expired:.*/expired: $new_exp/" "/etc/xray/database/trojan/${renew_user}.txt" 2>/dev/null; then
    ...
    response=$(cat <<EOF
{
    "status": "true",
    "code": 200,
    "message": "Trojan account '$renew_user' renewed successfully",
    "data": {
        "username": "$renew_user",
        "uuid": "$user_uuid",
...
if [[ "$1" == "--interactive" ]] || [[ "$1" == "-i" ]]; then
    ...
    renew_trojan_account "$renew_user" "$renew_days"
else
    ...
    # Eksekusi perpanjangan masa aktif account Trojan
    renew_trojan_account "$renew_user" "$renew_days"
fi