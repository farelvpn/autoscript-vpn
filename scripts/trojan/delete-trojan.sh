...
# Description: Delete TROJAN account (soft-delete, recoverable)
...
if ! db_account_exists "trojan" "$user"; then
...
if acc_xray_delete "trojan" "$user"; then
  tg_send "<b>[ TROJAN DELETED ]</b>%0AUsername: <code>${user}</code>%0AStatus: deleted (recoverable)"
