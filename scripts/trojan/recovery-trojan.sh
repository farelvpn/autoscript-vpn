...
echo -e "\e[0;41;36m              RECOVER TROJAN ACCOUNT                        \e[0m"
echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\e[037;1m"

# Check if recovery directory exists
if [[ ! -d "/etc/xray/recovery/trojan" ]]; then
...
recovery_files=$(ls /etc/xray/recovery/trojan/ 2>/dev/null)

if [[ -z "$recovery_files" ]]; then
    echo "No deleted TROJAN accounts found in recovery!"
    read -n 1 -s -r -p "Press any key to return to menu..."
    menu
    exit 0
fi
...
for recovery_file in $recovery_files; do
    if [[ -f "/etc/xray/recovery/trojan/$recovery_file" ]]; then
        username=$(echo "$recovery_file" | sed 's/\.txt$//')
        exp_date=$(grep "^expired:" "/etc/xray/recovery/trojan/$recovery_file" | cut -d' ' -f2-)
        deleted_date=$(stat -c %y "/etc/xray/recovery/trojan/$recovery_file" | cut -d' ' -f1)
...
if [[ ! -f "/etc/xray/recovery/trojan/${recover_user}.txt" ]]; then
...
username=$(grep "^username:" "/etc/xray/recovery/trojan/${recover_user}.txt" | cut -d' ' -f2-)
uuid=$(grep "^uuid:" "/etc/xray/recovery/trojan/${recover_user}.txt" | cut -d' ' -f2-)
limit_ip=$(grep "^limit_ip:" "/etc/xray/recovery/trojan/${recover_user}.txt" | cut -d' ' -f2-)
quota=$(grep "^quota:" "/etc/xray/recovery/trojan/${recover_user}.txt" | cut -d' ' -f2-)
expired=$(grep "^expired:" "/etc/xray/recovery/trojan/${recover_user}.txt" | cut -d' ' -f2-)

# Check if user already exists in current database
if [[ -f "/etc/xray/database/trojan/${recover_user}.txt" ]]; then
...
mv "/etc/xray/recovery/trojan/${recover_user}.txt" "/etc/xray/database/trojan/${recover_user}.txt"

# Recreate quota limit file if quota > 0
if [[ "$quota" -gt 0 ]]; then
    bytes=$((quota * 1024 * 1024 * 1024))
    echo "$bytes" > "/etc/xray/limit/quota/trojan/$recover_user"
fi

# Recreate IP limit file if limit_ip > 0
if [[ "$limit_ip" -gt 0 ]]; then
    echo "$limit_ip" > "/etc/xray/limit/ip/trojan/$recover_user"
fi

# Add user to config.json
sed -i '/#trojan$/a\#@ '"$recover_user $expired"'\
