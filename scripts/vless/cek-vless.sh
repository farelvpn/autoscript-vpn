...
# ========================================================
# ========================================================
# VLESS USER LOGIN INFO + TELEGRAM (Multi-Chunk HTML)
...
CLI_OUTPUT=$( {
    ...
    echo -e "\e[0;41;36m                   VLESS USER LOGIN INFO                   \e[0m"
    echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\e[037;1m"
    ...
    for akun in "${data[@]}"; do
        ...
        if [[ -s "$temp_log" ]]; then
            ...
            limit_ip="Unlimited"
            if [[ -f "/etc/xray/limit/ip/vless/$akun" ]] && val=$(cat "/etc/xray/limit/ip/vless/$akun") && [[ -n "$val" ]] && [[ "$val" != "0" ]]; then
                limit_ip="$val"
            fi
            
            limit_quota="Unlimited"
            if [[ -f "/etc/xray/limit/quota/vless/$akun" ]] && bytes=$(cat "/etc/xray/limit/quota/vless/$akun") && [[ "$bytes" -gt 0 ]]; then
                gb=$((bytes / (1024**3)))
                [[ $gb -gt 0 ]] && limit_quota="${gb} GB"
            fi
            
            usage_quota="0"
            if [[ -f "/etc/xray/usage/quota/vless/$akun" ]] && bytes=$(cat "/etc/xray/usage/quota/vless/$akun") && [[ "$bytes" -gt 0 ]]; then
