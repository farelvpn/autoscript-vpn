...
# Description: TROJAN IP-limit enforcement loop (DB-driven, grace threshold)
  ...
  while IFS='|' read -r user limit; do
    ...
    if [[ "$nets" -gt "$limit" ]]; then
      ...
      if [[ "${strikes[$user]}" -ge "$VIOLATION_THRESHOLD" ]]; then
        acc_xray_suspend "trojan" "$user" "iplimit"
        tg_send "<b>[ TROJAN IP LIMIT ]</b>%0AUsername: <code>${user}</code>%0AIPs: ${nets}/${limit}%0AStatus: suspended"
        unset 'strikes[$user]'
      fi
    else
      unset 'strikes[$user]'
    fi
  done < <(db_query "SELECT username, limit_ip FROM accounts WHERE protocol='trojan' AND status='active';")
  sleep 30
done
